...
class UpSample(nn.Module):
    ...
    def forward(self, input):
        return nn.functional.interpolate(input, scale_factor=self.scale_factor, mode='bilinear', align_corners=False)

class mysize(nn.Module):
    def __init__(self, img_size):
        super(mysize, self).__init__()
        self.img_size = img_size

    def forward(self, input):
        return nn.functional.interpolate(input, size = (self.img_size,self.img_size), mode='bilinear', align_corners=False)

class UpConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, scale_factor=4, kernel_size=1):
...
class finalconv(nn.Module):
    def __init__(self, in_channels, out_channels, img_size, kernel_size=1):
        super(finalconv, self).__init__()
        self.up_conv = nn.Sequential(
            mysize(img_size=img_size),
            nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=1),
            nn.ReLU(inplace=True)
        )
...
class EfficientNet(nn.Module):
    ...
    def forward(self, inputs):
        """EfficientNet's forward function.
           Calls extract_features to extract features, applies final conv layer, and returns logits.
        ...
        """
        bs = inputs.size(0) #batchsize

        # Convolution layers
        x = self.extract_features(inputs)

        # Pooling and final conv layer
